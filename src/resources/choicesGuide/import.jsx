...
export const importChoicesGuide = async function (apiUrl, httpClient, params) {
  ...
  data.forEach((line) => {
    if (
      line.choicesGuide_title ||
      line.choicesGuide_description ||
      line.choicesGuide_images ||
      line.choicesGuide_config
    ) {
      try {
    ...
    if (
      line.questionGroup_answerDimensions ||
      line.questionGroup_title ||
      line.questionGroup_description ||
      line.questionGroup_images ||
      line.questionGroup_seqnr
    ) {
      try {
    ...
    if (
      line.question_title ||
      line.question_description ||
      line.question_images ||
      line.question_type ||
      line.question_dimensions ||
      line.question_values ||
      line.question_seqnr
    ) {
      try {
    ...
    if (
      line.choice_title ||
      line.choice_description ||
      line.choice_images ||
      line.choice_type ||
      line.choice_dimensions ||
      line.choice_answers ||
      line.choice_seqnr
    ) {
      try {
  ...
  source.questionGroups.forEach(async (questiongroup) => {
    let result = await httpClient(
      `${apiUrl}/choicesguide/${CHOICESGUIDE_ID}/questiongroup`,
      {
        method: "POST",
        body: JSON.stringify({
    ...
    questiongroup.questions.forEach(async (question) => {
      let result = await httpClient(
        `${apiUrl}/choicesguide/${CHOICESGUIDE_ID}/questiongroup/${QUESTIONGROUP_ID}/question`,
        {
          method: "POST",
          body: JSON.stringify({
    ...
    questiongroup.choices.forEach(async (choice) => {
      let result = await httpClient(
        `${apiUrl}/choicesguide/${CHOICESGUIDE_ID}/questiongroup/${QUESTIONGROUP_ID}/choice`,
        {
          method: "POST",
          body: JSON.stringify({
            choicesGuideId: null,
  ...
  return {
    data: Object.assign(Object.assign({}, params.data), {
      id: CHOICESGUIDE_ID,
    }),
  };
};
