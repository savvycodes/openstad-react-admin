...
export const ResultButton = function(props) {
  ...
  let exporter = async function(data, id) {
    ...
    let json = await dataProvider.getChoicesgGuideResults({ id: props.data && props.data.id })
    ...
    jsonExport(body, {headers: [ 'userFingerprint', 'extraData', 'answers', 'scores' ]}, (err, csv) => {
  ...
  return (
    <Button label="Export resultaten" onClick={data => exporter(data, props.id)}>
      <Icon/>
    </Button>);
}
